...
const importDir = join(root, 'data/imports/inap-gestion-a2-2024');
...
const cleanPdfArtifacts = text => normalizeWhitespace(text)
  .replace(/\s*2024\s*-\s*GACE-L\s*Página\s+\d+\s+de\s+\d+\.?$/u, '')
...
const incomingPath = join(importDir, 'preguntas-inap-gestion-A2-2024.json');
const rawIncoming = JSON.parse(await readFile(incomingPath, 'utf8'));
if (!Array.isArray(rawIncoming)) throw new Error('El fichero A2 no contiene una lista de preguntas');
...
const incoming = rawIncoming.map(item => {
  ...
  question.origin = {
    ...
    type: 'official_exam',
    historical: true,
    label: question.origin?.label || 'INAP Gestión de la Administración Civil del Estado · A2 · 2024',
...
const report = {
  source: 'data/imports/inap-gestion-a2-2024',
...
console.log(`INAP Gestión A2 2024: ${imported.length} importadas, ${skippedDuplicates.length} duplicadas omitidas.`);
